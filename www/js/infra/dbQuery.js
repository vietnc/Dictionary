    ...
    try {
        this.db.transaction(function(tx) {
            tx.executeSql(sql, [], function(tx, results) {
                ...
                console.debug("exectute:"+ sql + "error: " + error);
                d.resolve(false);
            });
        }, function(error) {
...
DBAdapter.prototype.getFavList = function(){
    var sql = "SELECT id, word_id, title, content, speech, voice, meta FROM words INNER JOIN notebook on (words.id= notebook.word_id) ORDER BY added_date DESC";
    var result = this.query(sql);
    return result;
}
...
DBAdapter.prototype.saveWord = function(word){
    ...
    //     return this.query(sql)
    // }
     var sql = "INSERT OR IGNORE INTO notebook(word_id, n_view) VALUES(" + word.id + ",1)";
   return this.query(sql);
}
DBAdapter.prototype.addFav = function(noteb){
        var values = "'" + noteb.id + "', '" 
        + 1 + "', '" 
        + this.addSlashes(noteb.note) + "'" 
   ;
        var sql = "INSERT OR IGNORE INTO notebook(word_id, n_view, note) VALUES(" + values + ")";
        var sql2= " UPDATE notebook set n_view=n_view + 1, note='"+ this.addSlashes(noteb.note) + "', remembered_date='"+notb.remembered_date+"' WHERE word_id ="+ noteb.word_id
         this.query(sql) 
         this.query(sql2)
    return true;
}
DBAdapter.prototype.getLastFavWord = function(){
        var sql = "SELECT word_id, title, content, speech, voice, meta FROM words INNER JOIN notebook on (words.id= notebook.word_id) ORDER BY added_date DESC LIMIT 1";
        var result = this.query(sql);
        return result;
}
DBAdapter.prototype.getRandomFavWord = function(){
        var sql = "SELECT id, word_id, title, content, speech, voice, meta FROM words INNER JOIN notebook on (words.id= notebook.word_id) ORDER BY RANDOM() LIMIT 1";
        var result = this.query(sql);
        return result;
}
DBAdapter.prototype.removeFavWord = function(id){
    if(this.dbType == _DICT_TYPE_PERSONAL_){
        var sql = "DELETE FROM notebook WHERE word_id = '" + id + "'";
        return this.query(sql);
    }
    return true;
}
