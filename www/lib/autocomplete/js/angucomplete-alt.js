...
angular.module('angucomplete-alt', [] ).directive('angucompleteAlt', ['$parse', '$http', '$sce', '$timeout', '$anchorScroll' ,'$location', function ($parse, $http, $sce, $timeout, $anchorScroll, $location) {
    var KEY_DW = 40,
    KEY_UP = 38,
    KEY_ES = 27,
    KEY_EN = 13,
...
\n\
<icon-scroll><div id="{{id}}_dropdown" class="angucomplete-dropdown" ng-if="showDropdown"><div class="angucomplete-searching" ng-show="searching">Searching...</div>\n\
...
<div class="angucomplete-title" ng-if="!matchClass">{{ result.title }}</div><div ng-if="result.description && result.description != \'\'" class="angucomplete-description">{{result.description}}</div></div></div></ion-scroll></div>',
        link: function(scope, elem, attrs) {
            ...
            scope.selectResult = function(result) {
                ...
                $location.hash('search_input_value');
                $anchorScroll();                
                if (scope.matchClass) {
